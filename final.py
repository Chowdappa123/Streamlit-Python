...
def jmeter_results():
    ...
    selected_jtlfile = st.selectbox('Select a file to check the test results', jmeterResults)
    return os.path.join(selected_jtlfile)
...
def main():
    menu_list = ['Generate csv','Update jmx','Execute JMeter Test Plan','Test Results','Analyze JMeter Test Results', 'Recommendations','Home']
    ...
    if menu_sel=='Test Results' :
        st.title('Test Results')
        filename =  jmeter_results()
        ...
        #df = read_jtl_file(DATA_URL) 
        st.subheader('Request Summary')
        request_counts = data['Label'].value_counts()
        st.write(request_counts)  # Display the request counts as a table
        st.bar_chart(request_counts)
        # Calculate pass and fail percentages
        total_requests = data.shape[0]
        pass_count = data["success"].sum()
        fail_count = total_requests - pass_count
        pass_percentage = (pass_count / total_requests) * 100
      ...
      #  fig, ax = plt.subplots(figsize=(1, 1))
        ...
        st.write(f"Pass: {pass_count} ({pass_percentage:.1f}%)")
        st.write(f"Fail: {fail_count} ({fail_percentage:.1f}%)")
        ...
        errCount = filtered_data.groupby(['label', 'responseCode', 'errorMessage'])['responseCode'].count().reset_index(name='count')
